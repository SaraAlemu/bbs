...
esp32_url = "http://192.168.1.100/read"
...
try:
    ...
    logging.warning(f"Could not connect to MongoDB ({e}). Falling back to in-memory storage for users and readings.")
...
# Prefer TensorFlow models if available; otherwise fall back to scikit-learn pickles.
tf_available = False
try:
    import tensorflow as tf  # type: ignore
    tf_available = True
except Exception:
    logging.info("TensorFlow not available; will try scikit-learn models")

if tf_available:
    try:
        # Attempt to load TensorFlow model files
        model = tf.keras.models.load_model(os.path.join(base_dir, 'tensorflow_model.h5'))
        sc = pickle.load(open(os.path.join(base_dir, 'tensorflow_scaler.pkl'), 'rb'))
        le = pickle.load(open(os.path.join(base_dir, 'tensorflow_labelencoder.pkl'), 'rb'))
        logging.info("TensorFlow models loaded successfully")
    except Exception as e:
        logging.warning(f"Failed to load TensorFlow model files or dependencies: {e}; falling back to scikit-learn")
        tf_available = False
...
if not tf_available:
    try:
        ...
        logging.info("Scikit-learn models loaded as fallback")
    except FileNotFoundError:
        logging.error("No ML models found (neither TensorFlow nor scikit-learn artifacts were available)")
        exit(1)
...
def recommendation(features):
    try:
        ...
        # Use the tf_available flag set during startup to decide model type
        if tf_available:
            # TensorFlow model
            probabilities = model.predict(sc_features, verbose=0)[0]
        else:
            # Scikit-learn model
...
def load_user(user_id):
    try:
        # Load user from MongoDB by ObjectId
        user = users_collection.find_one({"_id": ObjectId(user_id)})
        if user:
            return User(str(user.get("_id")), user.get("username"))
    except Exception:
        pass
    return None
